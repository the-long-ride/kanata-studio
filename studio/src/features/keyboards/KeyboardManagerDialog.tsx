...
type ManagerProps = {
  ...
  onUpdate: (input: { id: string; name?: string; layoutOverride: KeyboardLayout | null }) => Promise<void>;
...
export function KeyboardManagerDialog(props: ManagerProps) {
  ...
  const key = `${props.keyboard.id}:${props.keyboard.name}:${props.keyboard.layoutOverride ?? 'Auto'}`;
...
function KeyboardManagerForm({
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="dialog keyboard-manager" onMouseDown={event => event.stopPropagation()}>
      <div className="keyboard-manager-head">
        <div><h3>Keyboard settings</h3><p className="muted">{keyboard.detectedName}</p></div>
      ...
      {!keyboard.configured ? <>
        ...
        <div className="keyboard-copy-block">
          ...
          <p className="muted">Replaces this keyboard&apos;s Global/app profiles and layout override.</p>
